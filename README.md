...
Existing installations that need loopback-only access should keep explicit bind settings when upgrading. After changing Compose configuration, rebuild and recreate Innkeeper with `docker compose up -d --build`.
